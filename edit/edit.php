...
$form->addVariable(_("Birthday"), 'user_birthday', 'monthdayyear', false, false, null, [date('Y') - 90, date('Y') - 10, '%Y%m%d', '%Y%m%d']);
$form->addVariable(_("Gender"), 'user_gender', 'enum', false, false, null, [[1 => _("Male"), 2 => _("Female")], _("--- Select ---")]);
$form->addVariable(_("City"), 'user_city', 'text', false);
$v = $form->addVariable(_("Country"), 'user_country', 'enum', false, false, null, [Folks::getCountries()]);
$v->setDefault('SI');
$form->addVariable(_("Homepage"), 'user_url', 'text', false);

if ($registry->hasMethod('video/listVideos')) {
    try {
        $result = $registry->call('video/listVideos', [['author' => $GLOBALS['registry']->getAuth()], 0, 100]);
        $videos = [];
        ...
        $form->addVariable(_("Video"), 'user_video', 'enum', false, false, $video_link, [$videos, _("--- Select ---")]);
...
$form->setButtons([_("Save"), _("Delete picture")]);
